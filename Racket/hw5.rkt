...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(call? e) (let ([f (eval-under-env (call-funexp e) env)])
                     (if (closure? f)
                         (eval-under-env (fun-body (closure-fun f)) (if (fun-nameopt (closure-fun f))
                                                                        (cons (cons (fun-nameopt (closure-fun f)) f)
                                                                              (cons (cons (fun-formal (closure-fun f))
...
(define (mlet* lstlst e2) (if (null? lstlst) e2
                              (let ([str (caar lstlst)]
                                    [vl (cdar lstlst)])
                                (mlet str vl (mlet* (cdr lstlst) e2)))))

(define (ifeq e1 e2 e3 e4) (mlet "_x" e1
                                 (mlet "_y" e2
                                       (ifgreater (var "_x") (var "_y")
                                                  e4 (ifgreater (var "_y") (var "_x")
                                                                e4 e3)))))

;; Problem 4

(define mupl-map (fun "mupl-map" "f" (fun #f "lst" (ifaunit (var "lst") (aunit)
