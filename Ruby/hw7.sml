...
datatype geom_exp = 
	 ...
	 | Var of string
	 | Shift of real * real * geom_exp (* function *)
...
fun preprocess_prog (e) =
	case e of
	(LineSegment l) => let val (x1, y1, x2, y2) = l
					in
						if real_close_point (x1, y1) (x2, y2) then Point(x1, y1)
						else if (x1 > x2 andalso not (real_close(x1, x2))) orelse 
						(real_close(x1, x2) andalso y1 > y2) 
